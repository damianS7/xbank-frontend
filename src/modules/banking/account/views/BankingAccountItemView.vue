...
import BankingAccountCards from "@/modules/banking/account/components/BankingAccountCardList.vue";
import BankingAccountTransactions from "@/modules/banking/account/components/BankingAccountTransactions.vue";
import { useTransactionStore } from "@/modules/banking/transaction/store/transaction";
import { useModalStore } from "@/stores/modal";
import Button from "@/components/ui/button/Button.vue";
import { Badge } from "@/components/ui/badge";
import type { BankingCardType } from "@/modules/banking/card/types/BankingCard";
import type { BankingAccountTransferForm } from "@/modules/banking/account/types/form/BankingAccountTransferForm";
...
async function setAlias(alias: string) {
  ...
  await accountStore
    .updateBankingAccountAlias(accountId, alias)
    .then(() => {
      alert.value?.success("Alias updated.", { timeout: 10 });
    })
    .catch((error) => {
      alert.value?.exception(error);
    });
